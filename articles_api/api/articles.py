...
@router.post("/articles/{article_id}/edit", response_model=Article) 
...
@router.post("/articles/{article_id}/add_authors") 
def add_authors(ids: List[int],
    ...
    if any(user_id in db_article.authors for user_id in ids):
...
@router.post("/articles/{article_id}/remove_authors") 
def remove_authors(ids: List[int],
    ...
    if len(crud.get_users_by_ids(db, ids)) < len(ids):
        raise HTTPException(status_code=409, detail="Ids list contains invalid user id")
    if any(user_id not in db_article.authors for user_id in ids):
...
@router.post("/articles/{article_id}/add_editors")
def add_editors(ids: List[int],
    ...
    if len(crud.get_users_by_ids(db, ids)) < len(ids):
        raise HTTPException(status_code=409, detail="Ids list contains invalid user id")
    if any(user_id in db_article.editors for user_id in ids):
...
@router.post("/articles/{article_id}/remove_editors") 
def remove_editors(ids: List[int],
    ...
    if len(crud.get_users_by_ids(db, ids)) < len(ids):
        raise HTTPException(status_code=409, detail="Ids list contains invalid user id")
    if any(user_id not in db_article.editors for user_id in ids):
